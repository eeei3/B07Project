...
public class EcoTrackerHomeFragment extends Fragment {
    ...
    private TextView transportEmissions, foodEmissions, shoppingEmissions, energyBills;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        TextView transportEmissions = view.findViewById(R.id.transport_emissions);
        TextView foodEmissions = view.findViewById(R.id.food_emissions);
        TextView shoppingEmissions = view.findViewById(R.id.shopping_emissions);
        TextView energyBills = view.findViewById(R.id.energy_bills);
    ...
    private void retrieveAndDisplayTotalsTraditional() {
        ...
        DatabaseReference rawInputsRef = databaseReference
                .child("users")
                .child(userId)
                .child("ecotracker")
                .child(String.valueOf(selectedDate))
                .child("rawInputs");//need to confirm this database

        // Retrieve totalTranspo
        rawInputsRef.child("totalTranspo").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (task.isSuccessful() && task.getResult().exists()) {
                    ...
                    transpoTextView.setText("Total Transport: " + totalTranspo);
                } else {
                    TextView transpoTextView = requireView().findViewById(R.id.transport_emissions);
                    transpoTextView.setText("Total Transport: Not logged yet");
                }
            }
        });

        // Retrieve totalFood
        rawInputsRef.child("totalFood").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (task.isSuccessful() && task.getResult().exists()) {
        ...
        rawInputsRef.child("totalShopping").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (task.isSuccessful() && task.getResult().exists()) {
        ...
        // Retrieve totalBills
        rawInputsRef.child("totalBills").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                if (task.isSuccessful() && task.getResult().exists()) {
